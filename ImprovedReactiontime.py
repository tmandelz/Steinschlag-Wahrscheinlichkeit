...
def Test(reactiontime, simsize):
    # Globale Variablen
    # Anzahl Durchgänge in der Monte Carlo Simulation
    # sizeMonteCarloSim = 500_000
    ...
    # Da wir in diesem Code unterschiedliche Häufigkeiten der Netzleerung überprüfen, ist der Parameter der floordivision variabel.
    listfeatures_samples["Tag"] = listfeatures_samples['CumsumHoursbeforeStone'].floordiv(reactiontime) # // 24
    ...
    for i in range(len(ListPossibleBrechFullNet)):
        ...
        ToCheck["NewTimeDiff"] = ToCheck["CumsumHoursbeforeStone"].diff()
    ...
    print("Resultat bei einer Reaktionszeit von", reactiontime, "Stunden:")
...
def Check(simsize):
    Reaktionszeiten = [4, 6, 8, 12, 14, 16, 20]
    resultat = []
    for reaktionszeit in Reaktionszeiten:
        resultat.append(Test(reaktionszeit, simsize))
    print(resultat)
